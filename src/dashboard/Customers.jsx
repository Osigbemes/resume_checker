...
export default function Dashboard() {
  ...
  const { data: responseData, isLoading: status, isError, } = useQuery(
    ["fetchCustomers", tableParams?.pagination, "/admin/business/customer"],
    fetchData, {
		onError: (err) => {
			alert(err);
		},
  ...
  // useEffect(() => {
  //   const myResponse = fetchMyCustomers();
  ...
  const popoverContentMenu = (
    <div>
      <Card>
        <div className='flex'>
          <CSVLink filename={"Customers-Huzz-file.csv"} data={data}>
            <Button style={{
              width: 90,
      ...
      <div className='w-1/2 m-auto flex justify-center'>
        <CardHeader color="green" contentPosition="none">
          <div className="w-full flex items-center justify-between">
            <h2 className="text-white text-2xl">Huzz Customers</h2>
            <div>
